...
class ContasReceberSiengeAPIClient:
    ...
    def buscar_dados_completos(self, 
        ...
        if data_fim is None:
            # Data fim dinâmica: mantém dia e mês de hoje, soma 40 anos ao ano
            data_fim_obj = date(hoje.year + 40, hoje.month, hoje.day)
            data_fim = data_fim_obj.strftime("%Y-%m-%d")
            logger.info(f"Data final definida automaticamente para: {data_fim} (hoje + 40 anos)")
