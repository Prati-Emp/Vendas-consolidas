#!/usr/bin/env python3
"""
Script para executar a atualização final da view no banco
"""
...
def executar_atualizacao_view(conn):
    """Executa a atualização da view principal"""
    print("\n" + "="*60)
    ...
    try:
        ...
        print("1. Verificando estrutura atual da view...")
        try:
            columns_atual = conn.execute("DESCRIBE informacoes_consolidadas.sienge_vendas_consolidadas").fetchall()
            print(f"   Colunas atuais: {len(columns_atual)}")
        except Exception as e:
            print(f"   View nao existe ou erro: {e}")
        
        # SQL para atualizar a view principal
        sql_principal = """
        ...
        print("2. Executando atualizacao da view principal...")
        ...
        print(f"   Colunas apos atualizacao: {len(columns_nova)}")
...
def main():
    """Funcao principal"""
    print("EXECUTANDO ATUALIZACAO FINAL DA VIEW")
    print("="*60)
    ...
    try:
        if not executar_atualizacao_view(conn):
            return False
        
        print("\n" + "="*60)
        print("ATUALIZACAO EXECUTADA COM SUCESSO!")
        print("="*60)
        print("A view sienge_vendas_consolidadas foi atualizada com as 3 novas colunas.")
