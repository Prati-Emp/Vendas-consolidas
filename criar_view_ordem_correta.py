#!/usr/bin/env python3
"""
Script para criar a view com a ordem correta das colunas
"""
...
def criar_view_ordem_correta(conn):
    """Cria a view com a ordem correta das colunas"""
    print("\n" + "="*60)
    print("CRIANDO VIEW COM ORDEM CORRETA DAS COLUNAS")
    print("="*60)
    
    try:
        # 1. Criar a view com a ordem correta das colunas
        print("1. Criando view com ordem correta das colunas...")
        ...
        # 4. Testar contagem
        print(f"\n4. Testando contagem de registros...")
        result = conn.execute("SELECT COUNT(*) FROM sienge_vendas_consolidadas").fetchone()
        print(f"   Total de registros: {result[0]:,}")
        
        # 5. Testar as novas colunas
        print(f"\n5. Testando novas colunas:")
        result = conn.execute("""
            SELECT 
                COUNT(*) as total,
        ...
        # 6. Mostrar estrutura completa
        print(f"\n6. Estrutura completa da view:")
...
def main():
    ...
    try:
        if not criar_view_ordem_correta(conn):
            return False
        
        print("\n" + "="*60)
        print("VIEW CRIADA COM SUCESSO!")
        print("="*60)
        print("A view sienge_vendas_consolidadas foi criada com as 3 novas colunas.")
