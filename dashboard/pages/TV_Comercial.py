...
# (Movido para baixo para garantir ordem correta de renderização e limpeza)
# carousel_placeholder = st.empty()  <-- Removido daqui
...
def render_velocimetro_metas(meta_valor: float, vendas_valor: float, atingimento_percent: float, mes_referencia: str):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge = {
            ...
            'steps': [
                {'range': [0, 70], 'color': 'rgba(30, 144, 255, 0.15)'},  # Azul muito mais transparente
                {'range': [70, 100], 'color': 'rgba(241, 196, 15, 0.15)'},  # Amarelo muito mais transparente
                {'range': [100, 150], 'color': 'rgba(255, 87, 34, 0.15)'}  # Laranja muito mais transparente
            ],
            'threshold': {
