...
# =============================================================================
# RESUMO – TERMÔMETRO DE VENDAS (apenas o termômetro)
# =============================================================================
st.markdown("---")
st.subheader("🌡️ Termômetro de Vendas (Resumo)")
...
try:
    ...
    CONVERSAO_SITUACOES = {s.lower() for s in ["Distrato", "Mútuo", "Vendida"]}
    SITUACOES_RESERVAS_EXCLUIDAS = {s.lower() for s in ["Mútuo", "Vencida"]}
    TERMOMETRO_SITUACOES_INATIVAS = {s.lower() for s in ["Cancelada", "Vendida", "Distrato"]}

    conn_tv = get_md_connection()
    reservas_sql = """
        SELECT
    ...
    """
    reservas_df = conn_tv.run_query(reservas_sql)
    ...
    mask = (
        ...
        ~reservas_df['situacao'].str.lower().isin(SITUACOES_RESERVAS_EXCLUIDAS)
    )
    base_df = reservas_df[mask].copy()
    base_df['situacao_normalizada'] = base_df['situacao'].str.lower()
    base_df['valor_contrato'] = pd.to_numeric(base_df.get('valor_contrato', pd.Series(dtype=float)), errors='coerce').fillna(0.0)

    reservas_ativas_df = base_df[~base_df['situacao_normalizada'].isin(TERMOMETRO_SITUACOES_INATIVAS)].copy()
    reservas_atuais_total = len(reservas_ativas_df)
    valor_total_reservas = float(reservas_ativas_df['valor_contrato'].sum())

    total_reservas_periodo = len(base_df)
    reservas_convertidas_total = base_df['situacao_normalizada'].isin(CONVERSAO_SITUACOES).sum()
    taxa_conversao_geral = (reservas_convertidas_total / total_reservas_periodo) if total_reservas_periodo > 0 else 0.0

    # Metas do mês atual (consolidado)
    mes_atual_col = {
        1: "jan/25", 2: "fev/25", 3: "mar/25", 4: "abr/25", 5: "mai/25", 6: "jun/25",
        7: "jul/25", 8: "ago/25", 9: "set/25", 10: "out/25", 11: "nov/25", 12: "dez/25"
    ...
    """
    metas_df = conn_tv.run_query(metas_sql)
    if mes_atual_col and not metas_df.empty and mes_atual_col in metas_df.columns:
        meta_total = float(pd.to_numeric(metas_df[mes_atual_col], errors='coerce').fillna(0).sum())
    else:
        meta_total = 0.0
    ...
    <div style='margin-top:0.5rem; position:relative; padding-top:36px;'>
      <div style='position:absolute; top:0; left:{indicador_posicao}%; transform:translateX(-50%); display:flex; flex-direction:column; align-items:center;'>
        <div style='font-size:0.7rem;font-weight:700;color:#ffffff;margin-bottom:6px;background:rgba(15,15,15,0.85);padding:2px 10px;border-radius:999px;'>{cobertura_percent:.1f}%</div>
        <div style='width:0;height:0;border-left:12px solid transparent;border-right:12px solid transparent;border-bottom:16px solid #ffffff;'></div>
      </div>
      <div style='position:relative; border-radius:14px; overflow:hidden; height:54px; box-shadow:0 0 8px rgba(0,0,0,0.15);'>
        <div style='display:flex; height:100%;'>
          <div style='flex:70; background:#1E90FF; color:#ffffff; display:flex; flex-direction:column; align-items:center; justify-content:center; font-weight:600; font-size:0.9rem;'>
            Frio
            <span style="font-weight:400;font-size:0.75rem;">&lt; 70%</span>
          </div>
          <div style='flex:30; background:#f1c40f; color:#0b0b0b; display:flex; flex-direction:column; align-items:center; justify-content:center; font-weight:700; font-size:0.9rem;'>
            Morno
            <span style="font-weight:500;font-size:0.75rem;">70% – 100%</span>
          </div>
          <div style='flex:50; background:#FF5722; color:#ffffff; display:flex; flex-direction:column; align-items:center; justify-content:center; font-weight:600; font-size:0.9rem;'>
            Quente
            <span style="font-weight:400;font-size:0.75rem;">&gt; 100%</span>
          </div>
        </div>
        <div style='position:absolute; top:0; bottom:0; left:{min(max(cobertura_percent/escala_max,0),1)*100}%; background:rgba(255,255,255,0.08); mix-blend-mode:screen;'></div>
    ...
    st.warning(f"⚠️ Não foi possível renderizar o termômetro: {e}")
