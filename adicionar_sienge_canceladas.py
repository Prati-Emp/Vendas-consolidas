#!/usr/bin/env python3
"""
Script simples para adicionar apenas a tabela Sienge Vendas Canceladas
"""
...
from scripts.sienge_apis import obter_dados_sienge_vendas_canceladas

async def adicionar_sienge_canceladas():
    """Adiciona apenas a tabela de vendas canceladas do Sienge"""
    print("🚀 ADICIONANDO SIENGE VENDAS CANCELADAS")
    print("=" * 50)
    ...
    try:
        ...
        print("\n4. Coletando dados do Sienge - Vendas Canceladas...")
        df_sienge = await obter_dados_sienge_vendas_canceladas()
        ...
        print("\n5. Criando tabela sienge_vendas_canceladas...")
        ...
        # Criar nova tabela
        conn.execute("CREATE TABLE main.sienge_vendas_canceladas AS SELECT * FROM df_sienge")
        print("✅ Nova tabela criada")
        
        # Verificar
        count = conn.sql("SELECT COUNT(*) FROM main.sienge_vendas_canceladas").fetchone()[0]
        print(f"✅ Tabela 'sienge_vendas_canceladas' criada com {count:,} registros")
        
        # 6. Mostrar algumas colunas
        print("\n6. Estrutura da tabela:")
        columns = conn.sql("DESCRIBE main.sienge_vendas_canceladas").fetchall()
        ...
        print(f"\n🎉 Sienge Vendas Canceladas adicionada com sucesso!")
...
if __name__ == "__main__":
    print("⚠️ ATENÇÃO: Este script irá adicionar apenas a tabela Sienge Vendas Canceladas")
    print("Pressione Ctrl+C para cancelar se necessário")
    print()
    
    try:
        sucesso = asyncio.run(adicionar_sienge_canceladas())
        
        if sucesso:
            print("\n✅ Tabela sienge_vendas_canceladas criada com sucesso!")
        else:
