#!/usr/bin/env python3
"""
Script para recriar a view principal sienge_vendas_consolidadas
"""
...
def recriar_view_principal(conn):
    """Recria a view principal sienge_vendas_consolidadas"""
    print("\n" + "="*60)
    print("RECRIANDO VIEW PRINCIPAL")
    print("="*60)
    
    try:
        # 1. Verificar se o schema informacoes_consolidadas existe
        print("1. Verificando schema informacoes_consolidadas...")
        try:
            conn.execute("CREATE SCHEMA IF NOT EXISTS informacoes_consolidadas")
            print("   Schema informacoes_consolidadas criado/verificado!")
        except Exception as e:
            print(f"   ERRO ao criar schema: {e}")
            return False
        
        # 2. Criar a view principal com as 3 novas colunas
        print("\n2. Criando view principal com as 3 novas colunas...")
        
        sql_view_principal = """
        CREATE VIEW informacoes_consolidadas.sienge_vendas_consolidadas AS
        ...
        conn.execute(sql_view_principal)
        print("   View principal criada com sucesso!")
        
        # 3. Verificar a nova estrutura
        print("\n3. Verificando nova estrutura da view...")
        columns_nova = conn.execute("DESCRIBE informacoes_consolidadas.sienge_vendas_consolidadas").fetchall()
        ...
        # 6. Testar as novas colunas
        print(f"\n6. Testando novas colunas:")
        result = conn.execute("""
            SELECT 
                COUNT(*) as total,
        ...
        # 7. Mostrar estrutura completa
        print(f"\n7. Estrutura completa da view:")
        for i, col in enumerate(columns_nova, 1):
            print(f"   {i:2d}. {col[0]} ({col[1]})")
        
        return True
        
    except Exception as e:
        print(f"ERRO ao recriar view principal: {e}")
        return False

def main():
    """Funcao principal"""
    print("RECRIANDO VIEW PRINCIPAL COM NOVAS COLUNAS")
    print("="*60)
    ...
    try:
        if not recriar_view_principal(conn):
            return False
        
        print("\n" + "="*60)
        print("VIEW PRINCIPAL RECRIADA COM SUCESSO!")
        print("="*60)
        print("A view sienge_vendas_consolidadas foi recriada com as 3 novas colunas.")
